...
import Objects.*;
import TSP_Solvers.EAConfig;
import java.util.*;
import java.util.concurrent.*;

/**
 * Population-based Evolutionary Algorithm 2. CO = Random; MU = Median;
 *
 * @author Xiaogang
 */
public class EA2 implements EA {
...
//            System.out.println(HC.getAverageFitness());
            System.out.println("##############################################");
            System.out.println("NAME : EA2_" + i);
            System.out.println("COMMENT : Excercise 2 EA2 Instance " + i++);
    ...
    public void start() {
        ...
        while (loopCount < config.maxGen) {
            ...
            for (int i = 0; i < config.maxPopulationSize / 2; i++) {
                nextGen.add(co.randomCrossover(population.get(i), population.get(i + 5)));
            }
            ...
            for (int i = 0; i < config.maxPopulationSize; i++) {
                nextGen.add(mu.MedianMutator(population.get(i), 0.5));
            }
